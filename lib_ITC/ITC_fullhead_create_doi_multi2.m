...
%20180424
%create doi for ERSP resulted from separate condition set files, so
%condition is the 4th dimension. and channel is the 3rd.
%IE.ERSP, nfreq x ntime x nchan x ncond x nsubj
%20180430, changed cluster.name to cluster.montage_name
%renamed cluster to chan_cluster
%set up oscillation_type as an individual input, since IE_struct has both
%ERSP and ITC
%added oscillation_type in variable name because need to combine ERSP and
%ITC
function doi = ITC_fullhead_create_doi_multi2(IE_struct,oscillation_type, ...
    chan_cluster,poi,export_text)

IE_struct.chan_cluster = chan_cluster;
IE_struct.oscillation_type = oscillation_type;


doi_mean = get_doi(IE_struct,poi,'mean',chan_cluster.channel);
doi_max = get_doi(IE_struct,poi,'max',chan_cluster.channel);
...
function doi = get_doi(IE_struct,poi,result_type,channel)
...
%modified on 20180424
%data = mean(IE_struct.(IE_struct.oscillation_type)(:,:,:,chan_cluster,:),4);
data = mean(IE_struct.(IE_struct.oscillation_type)(:,:,channel,:,:),3);
%[nfreqs,ntimes, ncond, ~,nsubj] = size(data);
[nfreqs,ntimes,~,ncond,nsubj] = size(data);
...
[extra_filename,extra_label] = construct_extra_names(IE_struct.chan_cluster.montage_name,IE_struct.group_name);
...
doi.chan_cluster = IE_struct.chan_cluster;
...
for i = 1:np
    ...
    if nc>1
        ...
        for n = 1:nc
            label_nosubject{m+n,1} = strcat(extra_label,...
                doi.oscillation_type,'_', type, '_',...
