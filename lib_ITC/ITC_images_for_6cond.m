...
function ITC_images_for_6cond(data_struct,channel,freq_range)
...
if nargin>2
   [freqs_adjusted_index, freqs_adjusted] = find_valid_pos(freq_range,freqs);
   freqs = freqs_adjusted;
   ERSP = ERSP(freqs_adjusted_index(1):freqs_adjusted_index(2),:,:);
   ITC = ITC(freqs_adjusted_index(1):freqs_adjusted_index(2),:,:);
end
...
channame = data_struct.channames{channel};
ERSP_labels = cell(1);
ITC_labels = cell(1);
for i = 1:nc
