...
function doi = ITC_fullhead_create_doi_multi(IE_struct,chan_cluster,poi,export_text)

chan_cluster = chan_cluster.channel;

IE_struct.cluster = chan_cluster;


doi_mean = get_doi(IE_struct,poi,'mean',chan_cluster);
doi_max = get_doi(IE_struct,poi,'max',chan_cluster);
...
data = mean(IE_struct.(IE_struct.oscillation_type)(:,:,chan_cluster,:,:),3);
...
[extra_filename,extra_label] = construct_extra_names(IE_struct.cluster.montage_name,IE_struct.group_name);
