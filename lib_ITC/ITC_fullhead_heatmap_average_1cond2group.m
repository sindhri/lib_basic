...
function ITC_fullhead_heatmap_average_1cond2group(ALLEEG,time_ranges,print_labels)

nplots = size(time_ranges,1);
ncond = length(ALLEEG);
if ncond~=2
    fprintf('program is only designed for 1cond 2groups\n');
    return
end
...
figure_limit = [data_limit;data_limit];
for i = 1:ncond
    f = figure;
    ...
    set(b,'position',[pos(1)+0.1,pos(2),colorbar_width_param,pos(4)],'clim',figure_limit(i,:));
