...
function ITC_single_file_vbaseline(category_names,baseline,...
    vbaseline,group_name,id_type,freqs,...
    pathname,result_foldername)

    if nargin==6
        pathname = uigetdir;
        pathname = [pathname '/'];
        
        result_foldername = [pwd '/result/'];
    end

    if ~exist(pathname,'dir')
        fprintf('pathname does not exist, abort.\n');
        return
    end
    ...
    for i = 1:length(file_list)
        ...
        if strcmp(temp(1),'.')~=1 && strcmp(temp(length(temp)-3:length(temp)),'.raw')
            ...
            [EEG,simple_count] = get_EEG_vbaseline(pathname,filename,...
                category_names,baseline,vbaseline,group_name,id_type);
            id_list{m} = EEG.id;
            trial_count = [trial_count; simple_count];
            ITC_calculation_single_file_vbaseline(EEG,freqs,result_foldername);
            m = m+1;
 %           if m == 2
 %               break
 %           end
