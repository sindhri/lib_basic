...
function [EEG, com] = pop_headplot_with_maplimit( EEG, typeplot, arg2, topotitle, rowcols, varargin);
...
maplimits_index = strmatch('maplimit',options(1:2:end));
if ~isempty(maplimits_index)
    maplimits = options{maplimits_index*2};
end
...
%            headplot( SIGTMPAVG(:,index), EEG.splinefile, 'maplimits', maplimits, options{:});
            headplot( SIGTMPAVG(:,index), EEG.splinefile, options{:});
