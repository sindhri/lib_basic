%20200918, change it to calculate only n cycle for studying delta, added an
%input variable cycle_n
...
%20181125, added vbaseline, usually [-600,-100];

function ITC_struct = ITC_calculation_vbaseline_ncycle(alleeg, chan_list,freq_limits,vbaseline,cycle_n)
...
[ERSP,~,~,times,freqs]=newtimef(mean(EEG.data(1,...
   ...
   EEG.srate, [cycle_n, 0.5], 'nfreqs',nfreqs, 'freqs', freq_limits,...
   'timesout',3000,'baseline',vbaseline,'plotitc','off',...
   'plotersp','off');
...
for i = 1:nsubjs
    ...
    for j = 1:nconds
        ...
        datapoint_end - datapoint_start + 1,calculation_time_range,...
        EEG.srate, [cycle_n, 0.5], 'nfreqs',nfreqs, 'freqs', freqs,...
       'timesout',ntimes,'baseline',vbaseline,'plotitc','off',...
       'plotersp','off');
