...
function count_trials
    ...
    for i = 1:length(file_list)
        ...
        if strcmp(temp(1),'.')~=1 && strcmp(temp(length(temp)-3:length(temp)),'.raw')
            ...
            if m == 1
                category_names = find_all_categories(EEG);
            end
...
function category_names = find_all_categories(EEG)

eventcategory = cell(1);
m = 1;
for i = 1:length(EEG.epoch)
    temp = EEG.epoch(i).eventcategory;
    if iscell(temp)
        eventcategory{m} = temp{1};
    else
        eventcategory{m} =temp;
    end
    m = m + 1;
end
category_names = unique(eventcategory);
