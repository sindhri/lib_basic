...
function [power,ntrials] = FFT_run_log_pick16(log_or_not)
...
for i = 1:length(file_list)
    ...
    if strcmp(temp(1),'.')~=1
       if strcmp(temp(length(temp)-3:length(temp)),'.mat');
           ...
           data = pick_trials(data);
 
           if ~isempty(data.picktrial_start)
                n = n+1;
                data = FFT_process_single_file_picktrial(data);
                data = FFT_average_single_file_log(data,log_or_not);
                if label_created == 0
                    power.label = FFT_generate_outputlabel(data,log_or_not);
                    label_created = 1;
                end
           ...
           ntrials.ids{m} = find_id(filename);
...
function data = pick_trials(data)
    n = data.ntrials;
    pick = 16;
    if n>=pick
        if rem(n,2)==0
            data.picktrial_start = n/2-pick/2+1;
            data.picktrial_end = n/2+pick/2;
        else
            data.picktrial_start = (n-1)/2-pick/2+1;
            data.picktrial_end= (n-1)/2+pick/2;
        end
    else
        data.picktrial_start =[];
        data.picktrial_end =[];
    end
...
function label = FFT_generate_outputlabel(data,log_or_not)
...
for i = 1:length(condition_names)
    ...
    for a = 1:length(channel_list_cell)
        ...
        for b = 1:length(frequency_index_cell)
            ...
            ave_name = [condition_names{i} '_cluster' int2str(a) chan_name 'freq' int2str(b) freq_name];
            if log_or_not==1
                ave_name = ['log_' ave_name];
            end
            label{m} = ave_name;
            m = m + 1;
        end
...
function data = FFT_average_single_file_log(data,log_or_not)
...
%    power = mean(power,3); %average across trials
    ...
    for a = 1:length(channel_list_cell)
        ...
        for b = 1:length(frequency_index_cell)
            ...
            if log_or_not==1
               power_chanave_freqave= log(power_chanave_freqave);
            end
