...
function item_pos = FH_plot_cor(corrintable,column_to_plot,items)
...
for i = 1:ncond
        corrEEG = corrintable(i,column_to_plot).(alldim{column_to_plot});
        if iscell(corrEEG)
            corrEEG = corrEEG{1,1};
        end
        ...
        for j = 1:length(items)
            ...
            a(j) = subplot(ncond,nitems,(i-1)*nitems+j);
...
%           data_plot= corrEEG.data(:,item_pos(j));
        ...
        set(b,'position',[pos(1)+0.1,pos(2),colorbar_width_param,pos(4)]);
