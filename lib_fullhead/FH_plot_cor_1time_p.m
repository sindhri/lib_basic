...
function FH_plot_cor_1time_p(report,chan_locs_file)
    if nargin==1
    ...
    set(f,'position',[0,0,res(3),res(4)]);
       data = report.p_sign;
            ...
            %top to bottom
            for i = 106:110
                bmap(i,:)=[1,1,1]; %p is between 0.06 and 0.1, yellow
            end
            for i = 102:105
                bmap(i,:)=[1,1,1]; %p is between 0.01 and 0.05, orange
            end
            bmap(101,:)=[1,1,1]; %p < .01, red
            bmap(100,:)=[1,1,1]; %p <.01 negative, blue
            for i = 96:99
                bmap(i,:)=[1,1,1]; %p between 0.01 and 0.05, negative, light blue
            end
            for i = 91:94
                bmap(i,:)=[1,1,1]; %p between 0.06 and 0.1, negative, lighter blue
            end
